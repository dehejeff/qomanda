...
import { buildMenuQrCardDataUrl } from '@/lib/table-qr-image'
...
/** QR do cardápio público — para imprimir na entrada do restaurante. */
export function MenuQrModal({ url, restaurantName, onClose }: Props) {
  ...
  function printCard() {
    ...
    win.document.write(`
      ...
      <body><img src="${cardDataUrl}" alt="QR Cardápio" onload="window.print()" /></body></html>
    `)
    win.document.close()
  }
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant">
          <div>
            ...
            <p className="text-xs font-mono text-on-surface-variant mt-0.5">Para imprimir na entrada do restaurante</p>
          </div>
        ...
        <div className="flex flex-col items-center px-6 py-6 gap-5">
          <div className="bg-white rounded-xl p-3 shadow-lg">
            {cardDataUrl ? (
              <img src={cardDataUrl} alt="QR Code Cardápio" className="w-full max-w-[320px]" />
            ) : (
          ...
          <p className="text-xs text-center font-mono text-on-surface-variant -mt-2">
            Coloque na entrada, no balcão ou em totens. O cliente escaneia, vê o cardápio e pode entrar na fila de espera.
          </p>
