...
import { buildCounterQrCardDataUrl } from '@/lib/table-qr-image'
...
/** QR do balcão — URL pública única (sem token por mesa). Para o restaurante imprimir e expor. */
export function CounterQrModal({ url, restaurantName, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function render() {
      ...
      const card = await buildCounterQrCardDataUrl(qrDataUrl, { restaurantName })
  ...
  function printCard() {
    ...
    win.document.write(`
      <!DOCTYPE html>
      <html><head><title>QR Balcão</title>
      <style>
        body { margin: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        img { max-width: 100%; height: auto; }
        @media print { body { margin: 0; } }
      </style></head>
      <body><img src="${cardDataUrl}" alt="QR Balcão" onload="window.print()" /></body></html>
    `)
    win.document.close()
  }
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-outline-variant">
          <div>
            ...
            <p className="text-xs font-mono text-on-surface-variant mt-0.5">Cliente escaneia e pede pelo celular</p>
          </div>
        ...
        <div className="flex flex-col items-center px-6 py-6 gap-5">
          <div className="bg-white rounded-xl p-3 shadow-lg">
            {cardDataUrl ? (
              <img src={cardDataUrl} alt="QR Code Balcão" className="w-full max-w-[320px]" />
            ) : (
          ...
          <p className="text-xs text-center font-mono text-on-surface-variant -mt-2">
            Imprima e coloque no balcão, caixa, totens ou no cardápio. Um único QR atende todos os clientes.
          </p>
