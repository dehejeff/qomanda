  ...
  return (
    <div className="rounded-xl overflow-hidden"
          ...
          {codes.map((p, i) => (
            <div key={`${p.confirmation_code}-${i}`} className="rounded-xl p-4 flex flex-col items-center gap-2"
              ...
              <div className="bg-white rounded-xl px-6 py-3 w-full text-center">
                <p className="text-3xl font-black tracking-widest" style={{ color: '#0D1117', fontFamily: 'Geist, sans-serif' }}>
                  {p.confirmation_code}
                </p>
              </div>
              <p className="text-xs font-mono" style={{ color: '#00E676' }}>{formatCurrency(p.amount)}</p>
            </div>
          ))}
        </div>
      ) : (
        <div className="px-5 pb-4">
          <p className="text-xs font-mono text-center py-3 rounded-xl" style={{ background: '#0D1117', color: '#8B949E', border: '1px solid #30363D' }}>
      ...
      <div className="px-5 pb-5">
        ...
        <p className="text-[10px] font-mono text-center mt-2 leading-relaxed" style={{ color: '#30363D' }}>
