...
//
// Config (env): LOAD_BASE, LOAD_VUS (20), LOAD_ITER (5), LOAD_TABLES (10),
//               LOAD_RESTAURANTS (1). Cenário roadmap: LOAD_RESTAURANTS=10 LOAD_TABLES=20.
//
...
const RESTAURANTS = Number(process.env.LOAD_RESTAURANTS || 1)
...
// ---- recursos de teste ----
const ownerIds = []
/** @type {{ id: string, itemIds: string[], tableIds: string[] }[]} */
const tenants = []
const customerIds = []
const sessionIds = []
...
async function seed() {
  for (let r = 0; r < RESTAURANTS; r++) {
    const email = `load-${STAMP}-${r}@smoke.com`
    ...
    ownerIds.push(u.user.id)
    const { data: rest, error: re } = await admin.from('restaurants').insert({
      owner_id: u.user.id, name: `Load ${STAMP} #${r + 1}`, slug: `load-${STAMP}-${r}`, status: 'active',
    }).select('id').single()
    if (re) throw new Error('restaurant: ' + re.message)
    const { data: cat } = await admin.from('menu_categories').insert({ restaurant_id: rest.id, name: 'Burgers', display_order: 1 }).select('id').single()
    const itemIds = []
    for (const [name, price] of [['Classic', 28], ['Cheddar', 32], ['Veggie', 26]]) {
      const { data: it } = await admin.from('menu_items').insert({ restaurant_id: rest.id, category_id: cat.id, name, price, available: true }).select('id').single()
      itemIds.push(it.id)
    }
    const tableIds = []
    for (let i = 1; i <= TABLES; i++) {
      const { data: t } = await admin.from('tables').insert({ restaurant_id: rest.id, number: String(i), status: 'free' }).select('id').single()
      if (t) tableIds.push(t.id)
    }
    tenants.push({ id: rest.id, itemIds, tableIds })
  }
  for (let i = 0; i < VUS; i++) {
    const { data: c } = await admin.from('customers').insert({
      first_name: 'Load', last_name: `${STAMP}_${i}`,
      whatsapp: '55' + STAMP.toString().slice(-8) + String(i).padStart(3, '0'),
    }).select('id').single()
    if (c) customerIds.push(c.id)
  }
  const totalTables = tenants.reduce((s, t) => s + t.tableIds.length, 0)
  console.log(`seed: ${tenants.length} restaurante(s) · ${totalTables} mesas · ${customerIds.length} clientes`)
}
...
async function journey(vu) {
  ...
  const tableId = tenant.tableIds[Math.floor(Math.random() * tenant.tableIds.length)]
  const customerId = customerIds[vu % customerIds.length]
  let sessionId = null
  await timed('1_checkin (session insert)', async () => {
    const res = await fetch(`${URL}/rest/v1/sessions`, { method: 'POST', headers: { ...h(SVC), Prefer: 'return=representation' }, body: JSON.stringify({ restaurant_id: tenant.id, table_id: tableId, customer_id: customerId, status: 'open' }) })
  ...
  await timed('2_menu (Supabase read)', async () => {
    const res = await fetch(`${URL}/rest/v1/menu_categories?select=*,items:menu_items(*)&restaurant_id=eq.${tenant.id}&order=display_order`, { headers: h(ANON) })
  ...
  await timed('4_order (Supabase write)', async () => {
    const res = await fetch(`${URL}/rest/v1/orders`, { method: 'POST', headers: { ...h(SVC), Prefer: 'return=representation' }, body: JSON.stringify({ restaurant_id: tenant.id, session_id: sessionId, customer_id: customerId, status: 'pending' }) })
    if (!res.ok) return false
    const orderId = (await res.json())[0]?.id
    const n = 1 + Math.floor(Math.random() * 3)
    const items = Array.from({ length: n }, () => {
      const id = tenant.itemIds[Math.floor(Math.random() * tenant.itemIds.length)]
      return { order_id: orderId, menu_item_id: id, quantity: 1 + Math.floor(Math.random() * 2), unit_price: 28 }
    })
...
async function main() {
  console.log(`\n=== Teste de carga · ${RESTAURANTS} restaurante(s) × ${TABLES} mesas · ${VUS} VUs × ${ITER} iter = ${VUS * ITER} jornadas · alvo ${BASE} ===`)
  await seed()
  const t0 = Date.now()
...
main()
  ...
  .finally(async () => {
    ...
    for (const t of tenants) await admin.from('restaurants').delete().eq('id', t.id)
    for (const id of customerIds) await admin.from('customers').delete().eq('id', id)
    for (const id of ownerIds) await admin.auth.admin.deleteUser(id).catch(() => {})
    console.log('cleanup: restaurantes (cascata) + clientes + owners removidos')
  })
